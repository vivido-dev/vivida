...
impl Shell {
    ...
    fn create_tab(&mut self, event_loop: &ActiveEventLoop) {
        self.create_tab_with_program(event_loop, None);
    }

    fn create_tab_with_program(
        &mut self,
        event_loop: &ActiveEventLoop,
        program: Option<&vivido::config::ui_config::Program>,
    ) {
        let cwd = self.active_pane_cwd();
        let mut terminal_options = self.terminal_options.clone();
        if let Some(program) = program {
            terminal_options.set_command(program);
        }
        let chrome = match self.chrome_window.as_ref().map(Arc::clone) {
            Some(chrome) => chrome,
            None => return,
        };
        let Ok(window_id) = NativePaneHost::new(chrome).create_pane(
            &mut self.processor,
            event_loop,
            &cwd,
            &terminal_options,
    ...
    fn activate_launch_entry(&mut self, event_loop: &ActiveEventLoop, index: usize) {
        ...
        match action {
            Some(LaunchAction::NewTab(program)) => {
                self.create_tab_with_program(event_loop, program.as_ref());
            }
