...
This file keeps what matters inside Vivida.
...
Inside a Vivida pane, position is ambient — the pane inherits `AGENT_MESH_RUNTIME=vivida`,
`AGENT_MESH_INSTANCE` (the `vivida-<pid>` automation name), and `AGENT_MESH_ADDRESS`:

```sh
vvagent bind --alias builder
```

A pane only ever learns its own **window**: `AGENT_MESH_ADDRESS` is `w<window_id>`, with no space or
tab. That is deliberate — a window id survives being dragged to another space, which is what makes
it the addressable part rather than any position in a tab strip. The space and tab are filled in
afterwards by the watcher's reconcile pass, which reads `vivida msg layout` and derives
`s<workspace_index>t<tab_index>w<window_id>` for every pane.

That arrangement is why Vivida needs no mesh code of its own. Its `layout` is already the
authoritative view, already accounts for reordering, and already has to be correct for every other
automation caller — so one reader cannot drift, where eight scattered index writes could each forget
...
Vivida owns `s`, `t`, and `w`. `f` and `p` appear only when a vvmux session is running *inside* one
of its panes, which is why the levels are ordered by containment rather than by runtime.
...
**Waking.** A watcher activates idle agents. Vivida starts one itself when `vvagent` is on PATH,
passing its own `vivida msg layout` as the reconcile command. Started by hand it looks like:

```sh
vvagent providers                                       # what can be woken on this machine
vvagent capabilities --native-session "$CODEX_THREAD"   # → activate_and_pull
vvagent watch --runtime vivida --instance NAME --parent-pid $$ \
  --reconcile "vivida msg layout"
```

One watcher per runtime instance. `--parent-pid` is its leash: it checks each pass and exits when
that process is gone, so no runtime needs a supervisor for it. `--reconcile` is what keeps addresses
true as panes move, since a pane's inherited environment cannot be edited after the fact.
`AGENT_MESH_WATCH=off` opts out; `AGENT_MESH_BIN` names the executable when it is not on PATH.
